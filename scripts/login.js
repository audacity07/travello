...
document.querySelector("#sign-up").addEventListener("click", function () {
  ...
  if (upPassword.value == "") {
    alert(
      "Looks like <Password> field is Empty! I dont think you want you account to be public"
    );
    return;
  }
  fetch(
    `https://users-mock-api.onrender.com/users?email_like=${upEmail.value}`,
    {
      method: "GET",
    }
  )
...
document.querySelector("#sign-in").addEventListener("click", function () {
  ...
  fetch(
    `https://users-mock-api.onrender.com/users?email_like=${inEmail.value}`,
    {
      method: "GET",
    }
  )
    ...
    .then((dataNew) => {
      ...
      for (let i = 0; i < dataNew.length; i++) {
        if (
          dataNew[i].email == "admin@gmail.com" &&
          dataNew[i].password == "admin"
        ) {
          showLoadingForAdmin();
          return;
        }
        if (
          dataNew[i].email == inEmail.value &&
          dataNew[i].password == inPass.value
        ) {
          console.log(
            "Found a user with same credentials, redirecting to landing page..."
          );
          localStorage.removeItem("userId");
...
function showLoadingForAdmin() {
  ...
  document.querySelector(".footer-outer-black-container").style.filter =
    "blur(8px)";
  setTimeout(function () {
